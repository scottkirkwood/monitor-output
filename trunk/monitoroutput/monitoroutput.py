...
class MonitorOutput:
    ...
    def handle_line(self, line):
        for event in self.curplugin.events:
            ...
            if match:
                if not self.dont_match(event, line):
    ...
    def dont_match(self, event, line):
        """ Returns true if part matches what we don't want to match

        """
        found_groups = None
        if 'unless' not in event or len(event['unless']) == 0:
            return None

        if 'compile_unless' not in event:
            if 'unless' in event:
                grep = '|'.join(event['unless'])
                event['compile_unless'] = re.compile(grep, event.get('params', 0))
            else:
                event['compile_unless'] = None

        if event['compile_unless'] == None:
            return None

        return  event['compile_unless'].search(line)
